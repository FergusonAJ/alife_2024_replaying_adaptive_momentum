...
head -n 1 ${SCRATCH_EXP_DIR}/reps/00001/cross_info.csv > ${CROSS_INFO_FILE}

# Actually scrape the data  
for REP_IDX in $( seq 1 10000 )
do
    ZERO_PADDED_IDX=$( ${REPO_ROOT_DIR}/global_shared_files/zero_pad.sh ${REP_IDX} 5 )
    printf " ${ZERO_PADDED_IDX}"
