...
SWEEP_INFO_FILE=../combined_sweep_info.csv
printf "" > ${SWEEP_INFO_FILE}

head -n 1 ${SCRATCH_EXP_DIR}/reps/001/cross_info.csv > ${SWEEP_INFO_FILE}

# Actually scrape the data  
for REP_IDX in $( seq 1 100 )
do
    ZERO_PADDED_IDX=$( ${REPO_ROOT_DIR}/global_shared_files/zero_pad.sh ${REP_IDX} 3 )
    echo "${ZERO_PADDED_IDX}"
    REP_SWEEP_INFO_FILE=${SCRATCH_EXP_DIR}/reps/${ZERO_PADDED_IDX}/cross_info.csv
    CROSS_INFO_LINES=$( wc -l ${REP_SWEEP_INFO_FILE} | grep -oP "^\d+"  )
    if [ ${CROSS_INFO_LINES} -gt 1 ]
    then
        tail -n $(( ${CROSS_INFO_LINES} - 1 )) ${REP_SWEEP_INFO_FILE} >> ${SWEEP_INFO_FILE}
    fi
done
